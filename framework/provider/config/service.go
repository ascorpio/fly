...
// 读取某个配置文件
func (conf *FlyConfig) loadConfigFile(folder string, file string) error {
	conf.lock.Lock()
...
// 删除文件的操作
func (conf *FlyConfig) removeConfigFile(folder string, file string) error {
	conf.lock.Lock()
	defer conf.lock.Unlock()
	s := strings.Split(file, ".")
	// 只有yaml或者yml后缀才执行
...
func NewFlyConfig(params ...interface{}) (interface{}, error) {
	...
	for _, file := range files {
		fileName := file.Name()
		err := flyConf.loadConfigFile(envFolder, fileName)
		if err != nil {
			log.Println(err)
			continue
		}
			...
			case ev := <-watch.Events:
				{
					...
					if ev.Op&fsnotify.Create == fsnotify.Create {
						...
						flyConf.loadConfigFile(folder, fileName)
					}
					...
					if ev.Op&fsnotify.Remove == fsnotify.Remove {
						...
						flyConf.removeConfigFile(folder, fileName)
					}
				}
...
// 通过path来获取某个配置项
func (conf *FlyConfig) find(key string) interface{} {
	conf.lock.RLock()
