...
import (
	...
	"github.com/ascorpio/fly/framework/provider/log/services"
)

// FlyLogServiceProvider 服务提供者
type FlyLogServiceProvider struct {
...
// Register 注册一个服务实例
func (l *FlyLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewFlyConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewFlySingleLog
	case "rotate":
		return services.NewFlyRotateLog
	case "console":
		return services.NewFlyConsoleLog
	case "custom":
		return services.NewFlyCustomLog
	default:
		return services.NewFlyConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *FlyLogServiceProvider) Boot(c framework.Container) error {
	return nil
}

// IsDefer 是否延迟加载
func (l *FlyLogServiceProvider) IsDefer() bool {
	return false
}

// Params 定义要传递给实例化方法的参数
func (l *FlyLogServiceProvider) Params(c framework.Container) []interface{} {
	// 获取configService
...
// Name 定义对应的服务字符串凭证
func (l *FlyLogServiceProvider) Name() string {
