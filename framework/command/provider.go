...
var providerListCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		flyContainer := container.(*framework.FlyContainer)
		// 获取字符串凭证
		list := flyContainer.NameList()
		// 打印
...
var providerCreateCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		// 检查服务是否存在
		providers := container.(*framework.FlyContainer).NameList()
...
import "github.com/ascorpio/fly/framework"
